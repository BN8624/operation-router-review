...
## 정적/격리 검증 (테스트 226/226 PASS, v2.4.6)
...
| 영수증 | owner/repo+canonical root SHA-256 namespace, clone 격리, review/repair 공통 verified provenance, REPAIR_REQUIRED review 필수, 수동 인수 assertion, legacy fail-closed |
| 실행 영속화 | active raw partial, terminal sanitization, 모든 최신 execution receipt 참조·active·불완전 marker 보호, 미참조 terminal retention 10, 보호 집합 실패 시 삭제 0 |
| recover | worker 호출 0회, 정상 result postflight 재개, result 유실 `*_unverified`, provenance 저장, unverified Sol review/repair 0회 |
| live probe | v2.4.6 유료 Grok·GPT·Claude live 호출 0회. v2.4.4의 과거 격리 Grok 1회는 역사 증거이며 이번 버전 PASS 근거로 승격하지 않음 |
...
| 외부 정적 검토 | v2.4.6은 사용자 지시에 따라 다른 Grok·Claude·Codex 세션 없이 직접 구현. 별도 외부 검토 미실행 |
