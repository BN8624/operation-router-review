...
$fixtureHome = Join-Path ([System.IO.Path]::GetTempPath()) ('operation-router-installed-v304-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    $testArguments = @(
        ...
        '-File', (Join-Path $PSScriptRoot 'run-tests.ps1'),
        '-RootPath', $routerRoot,
        '-SkillsPath', (Join-Path $routerRoot 'skills'),
        '-InstalledIntegration'
    )
    if (-not [string]::IsNullOrWhiteSpace($TestName)) {
        $testArguments += @('-TestName', $TestName)
    }
    ...
    if (-not $full.StartsWith($tempPrefix, [System.StringComparison]::OrdinalIgnoreCase) -or
        (Split-Path -Leaf $full) -notmatch '^operation-router-installed-v304-[a-f0-9]{32}$') { throw "refusing unsafe installed fixture cleanup: $full" }
