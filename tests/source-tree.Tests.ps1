...
Describe 'v2.3.4-1~17. 로그·상태·Skill·검토본 재현성' {
    ...
    It '12. README는 v3.0.14를 현재 버전으로 기록한다' {
        $readme = Get-Content -LiteralPath (Join-Path $RouterRoot 'README.md') -Raw -Encoding UTF8
        $readme | Should Match '^# operation-router \(v3\.0\.14\)'
    }
...
Describe 'v3.0.5 단계형 live canary와 verification drift' {
    ...
    It '34. REENTRY 버전 변조를 거부한다' {
        $root=New-VerificationMetadataFixture;[void]$script:VerificationFixtures.Add($root)
        $p=Join-Path $root 'REENTRY.md';(Get-Content -Raw -Encoding UTF8 $p).Replace('v3.0.14','v3.0.4')|Set-Content $p -Encoding UTF8
        (Invoke-VerificationMetadataTool $root).ExitCode|Should Be 1
    }
    It '35. VERIFICATION_MATRIX 버전 변조를 거부한다' {
        $root=New-VerificationMetadataFixture;[void]$script:VerificationFixtures.Add($root)
        $p=Join-Path $root 'VERIFICATION_MATRIX.md';(Get-Content -Raw -Encoding UTF8 $p).Replace('v3.0.14','v3.0.4')|Set-Content $p -Encoding UTF8
        (Invoke-VerificationMetadataTool $root).ExitCode|Should Be 1
    }
    It '36. CHANGELOG 최신 버전 변조를 거부한다' {
        $root=New-VerificationMetadataFixture;[void]$script:VerificationFixtures.Add($root)
        $p=Join-Path $root 'CHANGELOG.md';(Get-Content -Raw -Encoding UTF8 $p).Replace('## v3.0.14','## v3.0.4')|Set-Content $p -Encoding UTF8
        (Invoke-VerificationMetadataTool $root).ExitCode|Should Be 1
    }
