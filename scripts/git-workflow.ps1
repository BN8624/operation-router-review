...
function Test-ResealVerificationReceipt {
    ...
    if([string]::IsNullOrWhiteSpace($verification) -or $verification.Length -lt 12 -or
        $verification -match '(?i)^(current .+ session completed implementation|verification complete|verified|complete(d)?|done|success|tests? pass(ed)?|all tests pass(ed)?)$'){
