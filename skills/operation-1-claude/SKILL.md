...
4. 주문서에 실제 값으로 고정된 expected branch에서만 의미 단위 커밋을 만들고 expected remote branch에만 push한다.
5. 주문서의 구조화 완료 보고 계약대로 실제 current HEAD·operation·issue·work branch·검증 결과를 반환된 `workerReportPath` JSON에 저장한다.
6. 반환된 `postflightCommand`를 반드시 실행한다.
7. postflight 결과만 짧게 보고한다.

`claude_execute` JSON을 표시만 하고 끝내지 않는다. 재라우팅·재귀 handoff·다른 작업자 호출은 없다.

구현을 포기하고 lock만 남은 경우 `abandon-claude -Operation 1 -IssueNumber $0`을 사용한다. pending 누락·비Claude mutation purpose·dirty/HEAD 변경 증거가 있으면 거부된다.
...
## 출력
postflight JSON의 workflowMode/baseBranch/workBranch/prNumber/prUrl/prDraft/status/startHead/finalHead/commitCount/pushComplete/ciStatus/remainingProblems을 짧게 요약해 표시한다. 이슈 원문·장문 로그는 대화에 넣지 않는다.
