...
- **clone별 namespace와 보존**: namespace는 origin `owner/repo`와 canonical repository root의 SHA-256 앞 16자를 함께 사용한다. 같은 origin의 복수 clone도 execution·lock·run/review receipt를 공유하지 않는다. exact canonical root가 확인된 비활성 legacy receipt만 새 namespace로 원자 이전하며, terminal generation은 namespace별 최근 10개를 보존하고 active·최신 참조 generation은 삭제하지 않는다.
...
├─ tests/source-tree.Tests.ps1 # 기존 회귀 + v2.4.5 보안 강화 격리 테스트
...
v2.4.5 검증은 Pester mock/fake Git/격리 OS process와 임시 사용자 홈 installed fixture만 사용한다. 유료 Grok·GPT live 호출은 실행하지 않았다. v2.4.4의 과거 격리 Grok live probe 1회는 이번 버전의 live 검증으로 승격하지 않는다.
