...
† sol 역할의 설계 모델은 `gpt-5.6-sol`이다. 2026-07-21 기준 **주 사용 Codex 계정은 한도 소진으로 Sol을 실행할 수 없었고, 별도 E2E 테스트 계정에서는 Sol이 models_cache에 노출되지 않아 doctor가 unresolved로 판정**했다. 이에 따라 **테스트 환경에서만** 사용자 지시로 sol 역할을 `gpt-5.6-terra`에 임시 매핑했다 (config `gpt.workers.sol`·`_sol_note`). 이는 Sol이 전역 제거·폐기됐다는 뜻이 아니며 Terra가 공식 대체자도 아니다. Sol canon은 유지하며, 한도가 복구되고 Sol이 노출되는 계정에서 V11~V13·V15를 재검증한다.
...
- Codex 0.144.5: `codex exec --cd -m -c model_reasoning_effort=<e> -s workspace-write -c approval_policy=never -c sandbox_workspace_write.network_access=true --json -` (프롬프트 stdin). `-a`는 `codex exec`에 없는 옵션이므로 쓰지 않는다. 모델 `gpt-5.6-terra`/`gpt-5.6-luna` (`~/.codex/models_cache.json` 실측). `gpt-5.6-sol`은 E2E 테스트 계정의 models_cache에 노출되지 않아 그 계정에서 doctor가 `unresolved`로 보고하고 unresolved 모델 호출은 fail-closed로 차단된다. 이는 해당 테스트 계정 상황 + 주 계정 한도 소진을 반영한 것이지 Sol의 전역 제거가 아니다. 작전 1의 Sol 경로(V11~V13·V15)는 한도 복구·Sol 노출 계정에서 재검증한다(Sol canon 유지).
...
- **작전 1 Sol 재검증 대기**: GPT Plan B(Luna/Terra)와 Claude-only 경로는 실전 E2E 통과. 작전 1의 Sol 역할은 테스트 환경에서 Terra로 임시 매핑해 검증했으므로(V11~V13·V15 = PASS_PENDING_SOL_RETEST), 한도 복구·Sol 노출 계정에서 재검증 후 최종 PASS로 승격한다. 상세는 [VERIFICATION_MATRIX.md](VERIFICATION_MATRIX.md).
