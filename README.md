...
→ final review
→ CI success
→ merge_ready + Draft 해제
```

`merge_ready`는 병합 완료가 아니다. 라우터는 Draft를 ready-for-review로 바꿀 수 있지만 자동 merge, merge queue, branch 삭제, local main fast-forward, rebase, conflict 해결을 하지 않는다. 실제 병합은 사용자가 GitHub에서 별도로 수행한다.
...
- worker는 마지막 줄의 엄격한 `[ORH_WORKER_REPORT]` JSON으로 실제 로컬 검증 완료 여부, 검증 요약, 남은 문제를 보고한다. 마커가 없거나 잘못되면 로컬 검증 완료를 추측하지 않으며 `merge_ready`를 만들지 않는다.
- PR CI는 PR 번호와 현재 PR head SHA의 모든 check run/status context를 집계한다. 대상 저장소의 workflow가 `pull_request`를 지원해야 하며 operation-router는 대상 프로젝트의 Actions 파일이나 branch protection을 자동 변경하지 않는다.
...
Operation 1의 Opus 종료 검토 또는 Operation 2의 Sonnet 종료 검토가 끝나면 Skill이 `finalize`에 verdict를 전달한다. PASS여도 PR context, clean worktree, branch push, artifact, 경계 검사, CI success를 모두 만족해야 `merge_ready`가 되며 이때만 Draft를 해제한다. `REPAIR_REQUIRED`, CI pending/failure/unavailable은 Draft를 유지한다.
...
`pull-request`는 PR 번호와 PR head SHA에 연결된 모든 check run과 status context를 조회한다. 하나라도 failure/cancelled/timed_out/action_required면 `failure`, 실패가 없고 queued/pending/in_progress가 하나라도 있으면 `pending`, 하나 이상 존재하고 모두 명시적 success면 `success`다. neutral/skipped/알 수 없는 conclusion, API·인증·JSON 오류, workflow가 있는데 polling 종료까지 check가 없으면 `unavailable`이다. workflow 파일과 check가 모두 없을 때만 `not-requested`다. 첫 check 하나만 보고 성공 처리하지 않는다.
